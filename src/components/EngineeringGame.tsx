...
const translations = {
  en: {
    ...
    buildRobotDesc:
      "Assemble robot parts in the correct order!",
    ...
    marbleRunDesc:
      "Design ramps and guide the marble to the goal.",
    playNow: "Play Now",
  ...
  hi: {
    ...
    bridgeBuilderDesc:
      "प्लेटफार्मों को जोड़ने के लिए पुल बनाएं!",
  ...
  od: {
    ...
    bridgeBuilderDesc:
      "ପ୍ଲାଟଫର୍ମଗୁଡ଼ିକୁ ସଂଯୋଗ କରିବା ପାଇଁ ବ୍ରିଜ୍ ନିର୍ମାଣ କରନ୍ତୁ!",
    simpleMachine: "ସରଳ ମେସିନ୍ ମ୍ୟାଚ୍",
    simpleMachineDesc:
      "ମେସିନ୍ଗୁଡ଼ିକୁ ତାଙ୍କର ବ୍ୟବହାର ସହିତ ମେଳାନ୍ତୁ ।",
...
export function EngineeringGame({
  ...
  if (currentGame.startsWith("how-to-")) {
    ...
    const gameInstructions =
      instructions?.[
        validLanguage as keyof typeof instructions
      ] || instructions?.en;
          ...
          <Card className="border-2 shadow-xl">
            ...
            <CardContent className="p-6 md:p-8">
              ...
              <div className="space-y-3">
                {gameInstructions?.steps.map(
                  (step: string, index: number) => (
                    <div
                      key={index}
                      className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors"
                    >
                      <span className="shrink-0 w-6 h-6 bg-linear-to-r from-purple-500 to-pink-500 text-white rounded-full flex items-center justify-center text-sm">
                        {index + 1}
                      </span>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mt-4 text-center text-xs md:text-sm text-gray-600">
          <p className="hidden md:block">
            Use mouse to drag and click • Use keyboard when
            needed
          </p>
          <p className="md:hidden">
            Tap and drag to interact • Use touch buttons when
            available
          </p>
        </div>
      </div>
...
class BuildRobotScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add
      .text(width / 2, 30, "🤖 Build the Robot!", {
        fontSize: "24px",
        color: "#fff",
      })
      .setOrigin(0.5);
    ...
    this.statusText = this.add
      .text(
        width / 2,
        70,
        `Start with the ${this.sequence[0].toUpperCase()}`,
        {
          fontSize: "18px",
          color: "#ffd",
        },
    ...
    const partNames = Phaser.Utils.Array.Shuffle([
      ...this.sequence,
    ]);
  ...
  private createRobotPart(name: string, x: number, y: number) {
    ...
    const shapes: any = {
      ...
      "lower-body": () =>
        this.add.rectangle(0, 0, 60, 40, colors["lower-body"]),
      body: () => this.add.rectangle(0, 0, 60, 50, colors.body),
      "upper-body": () =>
        this.add.rectangle(0, 0, 65, 40, colors["upper-body"]),
      shoulders: () => {
    ...
    const label = this.add
      .text(0, 35, name.toUpperCase().replace("-", " "), {
        fontSize: "10px",
        color: "#fff",
      })
      .setOrigin(0.5);
    ...
    container.on("pointerdown", () =>
      this.tryPlacePart(name, container),
    );
  ...
  private tryPlacePart(
    ...
    if (name === expectedPart) {
      ...
      this.tweens.add({
        ...
        onComplete: () => {
          ...
          if (this.currentIndex >= this.sequence.length) {
            ...
            const nextPart = this.sequence[this.currentIndex]
              .toUpperCase()
              .replace("-", " ");
      ...
      this.statusText.setText(
        `❌ Wrong! Need ${expectedPart.toUpperCase().replace("-", " ")} first`,
      );
...
class BridgeBuilderScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Title
    this.add
      .text(width / 2, 30, "🌉 Build the Bridge!", {
        fontSize: "24px",
        color: "#fff",
      })
      .setOrigin(0.5);
    ...
    this.statusText = this.add
      .text(width / 2, 60, "Drag beams to build your bridge", {
        fontSize: "16px",
        color: "#ffd",
      })
      .setOrigin(0.5);
    ...
    const leftPlat = this.add.rectangle(
      this.leftPlatX,
      groundY - 50,
      100,
      100,
    ...
    const rightPlat = this.add.rectangle(
      this.rightPlatX,
      groundY - 50,
      100,
      100,
    ...
    this.add
      .text(this.rightPlatX, groundY - 100, "🏁", {
        fontSize: "32px",
      })
      .setOrigin(0.5);
    ...
    this.input.on(
      ...
      (pointer: any, gameObject: any) => {
        if (
          !this.isTestMode &&
          this.beamPalette.includes(gameObject)
        ) {
  ...
  private createControlButtons() {
    ...
    this.testBtnText = this.add
      .text(this.scale.width - 120, btnY, "TEST", {
        fontSize: "18px",
        color: "#fff",
      })
      .setOrigin(0.5)
    ...
    this.add
      .text(this.scale.width - 240, btnY, "CLEAR", {
        fontSize: "18px",
        color: "#fff",
      })
      .setOrigin(0.5)
  ...
  private clearBridge() {
    ...
    this.placedBeams = [];
    this.statusText.setText(
      "✅ Bridge cleared! Build a new one",
    );
  ...
  private checkBridgeResult() {
    ...
    if (
      vehicleX >= this.rightPlatX - 70 &&
      vehicleY < 420 &&
      (!body || body.velocity.y <= 1)
    ) {
      this.bridgeSuccess();
      return;
    }
  ...
  private bridgeSuccess() {
    ...
    this.add
      .text(this.scale.width / 2, 250, "🎉 Mission Complete!", {
        fontSize: "32px",
        color: "#76ff03",
        stroke: "#000",
        strokeThickness: 4,
  ...
  private bridgeFailed() {
    ...
    this.add
      .text(this.scale.width / 2, 250, "💥 Mission Failed!", {
        fontSize: "32px",
        color: "#ff6b6b",
        stroke: "#000",
        strokeThickness: 4,
  ...
  private onSelectChoice(
    ...
    if (correct) {
      ...
      if (
        ...
        this.awaiting.textObj.active &&
        this.awaiting.textObj.canvas
      ) {
        try {
          ...
          console.warn(
            "Failed to reset text style after wrong answer:",
            e,
          );
        }
      }
...
class MarbleRunScene extends Phaser.Scene {
  ...
  create() {
    ...
    const difficultyText =
      this.difficulty === 1
        ...
        : this.difficulty === 2
          ...
          : "Hard";
    this.add
      .text(width / 2, 18, `Difficulty: ${difficultyText}`, {
        color: "#ffd700",
        fontSize: "16px",
        fontStyle: "bold",
      })
    ...
    this.add
      .text(this.goalX, this.goalY, "🎯", { fontSize: "24px" })
      .setOrigin(0.5);
    ...
    this.physics.add.collider(
      this.marble,
      startPlatform as unknown as Phaser.GameObjects.GameObject,
    );
    this.physics.add.collider(
      this.marble,
    ...
    const checkEvent = this.time.addEvent({
      ...
      callback: () => {
        ...
        if (
          ...
          if (this.marble.body) {
            (
              this.marble.body as Phaser.Physics.Arcade.Body
            ).setVelocity(0, 0);
            (
              this.marble.body as Phaser.Physics.Arcade.Body
